...
String convert(uint16_t code);
String convertLong(uint16_t code);
String convertShort(uint8_t code);
void analizeInputBuffer();
...
uint8_t crunchPacket()
{
    ...
    {
        Serial.println("partity error");
        data = -1; // Parity not ok
    }

    return (!startBit && stopBit) ? data : 0;
}
...
String convert(uint16_t code)
{
    if ((code & 0xFF00) == (KBRD_LONG_CODE << 8))
        return convertLong(code);
    else
        return convertShort(code & 0x00FF);
}

String convertLong(uint16_t longCode)
{
    switch (longCode)
    {
    case KBRD_KEY_L_WIN:
        return "lwin";
    case KBRD_KEY_R_CTRL:
        return "rctrl";
    case KBRD_KEY_R_WIN:
        return "rwin";
    case KBRD_KEY_R_ALTGR:
        return "raltgr";
    case KBRD_KEY_APPS:
        return "apps";
    case KBRD_KEY_INSERT:
        return "insert";
    case KBRD_KEY_HOME:
        return "home";
    case KBRD_KEY_PG_UP:
        return "pgup";
    case KBRD_KEY_DELETE:
        return "del";
    case KBRD_KEY_END:
        return "end";
    case KBRD_KEY_PG_DOWN:
        return "pgdown";
    case KBRD_KEY_UP:
        return "up";
    case KBRD_KEY_LEFT:
    ...
    case KBRD_KEY_RIGHT:
        return "right";
    case KBRD_KEY_KP_ENTER:
        return "kpenter";
    case KBRD_KEY_KP_SLASH:
        return "kp/";
    case KBRD_KEY_SLEEP:
        return "sleep";
    case KBRD_KEY_WAKE:
...
String convertShort(uint8_t shortCode)
{
    switch (shortCode)
    {
    case KBRD_KEY_0:
    ...
    case KBRD_KEY_MINUS:
        return "-";
    case KBRD_KEY_ANGL_BRKT:
        return "<";
    case KBRD_KEY_BCK_SLASH:
        return "\\";
    case KBRD_KEY_QSTN_MARK:
        return "'";
    case KBRD_KEY_CAP:
        return "ì";
    case KBRD_KEY_L_SQRBRKT:
        return "è";
    case KBRD_KEY_R_PLUS:
        return "+";
    case KBRD_KEY_AT:
        return "ò";
    case KBRD_KEY_HASHTAG:
        return "à";
    case KBRD_KEY_ACCNT_U:
        return "ù";
    case KBRD_KEY_BKSP:
        return "bksp";
    case KBRD_KEY_SPACE:
    ...
    case KBRD_KEY_CAPS:
        return "caps";
    case KBRD_KEY_L_SHIFT:
        return "lshift";
    case KBRD_KEY_L_CTRL:
        return "lctrl";
    case KBRD_KEY_L_ALT:
        return "lalt";
    case KBRD_KEY_R_SHIFT:
        return "rshift";
    case KBRD_KEY_ENTER:
        return "enter";
    case KBRD_KEY_ESC:
    ...
    case KBRD_KEY_F12:
        return "f12";
    case KBRD_KEY_SCROLL:
        return "scroll";
    case KBRD_KEY_NUM:
        return "num";
    case KBRD_KEY_KP_ASTX:
        return "kp*";
    case KBRD_KEY_KP_MIN:
        return "kp-";
    case KBRD_KEY_KP_PLUS:
        return "kp+";
    case KBRD_KEY_KP_DOT:
        return "kp.";
    case KBRD_KEY_KP_0:
        return "kp0";
    case KBRD_KEY_KP_1:
...
void analizeInputBuffer()
{
    // If the buffer is empty skip all
    if (!available())
        return;

    // Read the third byte of a long break code
    if (readingLongCode)
    {
        uint16_t longCode = (KBRD_LONG_CODE << 8) | read();
        readingLongCode = false;

        Serial.print("\t\t\t[^] Released long key: ");
        Serial.println(convert(longCode));
        return;
    }

    // Check if it's a long code
    if (peek() == KBRD_LONG_CODE)
    {
        if (available() == 1)
            return;
        ...
        if (next == KBRD_BREAK)
        {
            readingLongCode = true;
            return;
        }

        uint16_t longCode = (KBRD_LONG_CODE << 8) | next;
        Serial.print("\t\t\t[v] Pressed long key: ");
        Serial.println(convert(longCode));
        return;
    }
